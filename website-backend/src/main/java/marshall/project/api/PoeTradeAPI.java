...
public class PoeTradeAPI {
	...
	public ResponseEntity<List<TradeItem>> getTradeItems() {
		try {
			List<TradeItem> tradeItems = poeTradeService.getTradeItems();
	...
	public ResponseEntity<Integer> addTradeItem(@RequestBody TradeItem tradeItem) {
		System.out.println("hello" + tradeItem.getDescription());
		try {
